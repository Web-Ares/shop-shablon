...
class JCKWooThumbs {
    ...
    public function save_product_fields( $post_id ) {
        ...
        // Remove product default variation ID transient, so it resets
        // the next time the product page is loaded, in case it has changed
        $transient_name = sprintf('%s-dvi-%d', $this->slug, $post_id);
     ...
     * @param bool $force
     */
    public function delete_transients( $force = false ) {
    ...
    public function register_scripts_and_styles() {
        ...
        if ( ( ( function_exists('is_product') && is_product() ) || has_shortcode( $post->post_content, 'product_page' ) ) && $this->is_enabled() ) {
    ...
    public function enqueue_photoswipe() {
        ...
        if( isset( $jck_wt['enableLightbox'] ) && $jck_wt['enableLightbox'] ) {
    ...
    /**
     *
     * EOutput photoswipe HTML template
     *
     */
    ...
    private function load_file( $handle, $file_path, $is_script = false ) {
        ...
        if ( file_exists( $file ) ) {
            if ( $is_script ) {
                wp_register_script( $handle, $url, array('jquery'), false, true ); //depends on jquery
    ...
    public function get_default_variation_id() {
        ...
        $transient_name = sprintf('%s-dvi-%d', $this->slug, $product->id);
